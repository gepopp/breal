...
return [
    ...
    */
    ...
    | Remote access token
    |--------------------------------------------------------------------------
    | Shared secret that a central Log Viewer instance must send as a Bearer
    | token to read THIS app's logs remotely. It is validated in the
    | LogViewer::auth() callback (see AppServiceProvider) — remote requests
    | carry no authenticated session, so the `viewLogViewer` gate alone would
    | reject them with a 403.
    |
    */

    'remote_access_token' => env('LOG_VIEWER_REMOTE_ACCESS_TOKEN'),
