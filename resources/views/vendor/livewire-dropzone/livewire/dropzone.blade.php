    ...
    <div class="flex flex-col items-start h-full w-full justify-center bg-transparent dark:border-gray-600 dark:hover:border-gray-500">
        ...
        <div @click="$refs.input.click()" class="border border-dashed rounded border-gray-500 w-full cursor-pointer">
            <div>
                <div x-show="!isDragging" class="flex items-center bg-gray-50 justify-center gap-3 py-8 h-full dark:bg-gray-700">
                    ...
                    <p class="!text-sm md:text-base text-gray-600 dark:text-gray-400">Hier ablegen oder <span class="font-semibold text-black dark:text-white">Dateien suchen</span></p>
                </div>
                <div x-show="isDragging" class="flex items-center bg-gray-100 dark:bg-gray-800 justify-center gap-3 py-8 h-full">
                    ...
                    <p class="!text-sm md:text-base text-gray-600 dark:text-gray-400">Zum Hochladen ablegen</p>
                </div>
            </div>
            <input
                    x-ref="input"
        ...
        <div class="flex justify-between w-full mt-2">
            <div class="flex gap-3 text-gray-500 text-xs md:text-sm">
                ...
                @if($hasMaxFileSize)
                    <p class="!text-xs">{{ __('Bis zu :size pro Datei', ['size' => \Illuminate\Support\Number::fileSize($this->maxFileSize * 1024)]) }}</p>
                @endif
                ...
                @if($hasMimes)
                    <p class="!text-xs">{{ Str::upper($this->mimes) }}</p>
                @endif
            </div>
            <div x-show="isLoading" class="flex gap-1 items-center">
                ...
                <div @click="cancelUpload" class="text-xs md:text-sm text-gray-800 dark:text-gray-200 hover:cursor-pointer underline">abbrechen</div>
            </div>
        </div>
                    ...
                    <div class="flex items-center justify-between gap-2 border rounded border-gray-200 w-full h-auto overflow-hidden dark:border-gray-700">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex flex-col items-start gap-1">
                                <div class="text-start line-clamp-1 text-slate-900 text-xs md:text-sm font-medium dark:text-slate-100">{{ $file['name'] }}</div>
                                <div class="text-start text-gray-500 text-xs md:text-sm font-medium">{{ \Illuminate\Support\Number::fileSize($file['size']) }}</div>
                            </div>
                        </div>
            ...
            return ({
                ...
                init() {
                    this.$watch('isLoading', (isLoading) => {
                        console.log('livewire-dropzone isLoading', isLoading);
                        this.$dispatch('upload', isLoading)
                    })
                },
                onDrop(e) {
                    this.isDropped = true
